...
class PermissionsHandler(private val context: Context) {
    ...
    fun requestHistoryReadPermission(activity: ComponentActivity) {
        try {
            // Launch Health Connect app with specific intent
            val intent = Intent("android.health.connect.action.REQUEST_PERMISSION").apply {
                putExtra(
                    "android.health.connect.extra.REQUEST_PERMISSIONS",
                    arrayOf(historyReadPermission)
                )
            }
    ...
    fun requestBackgroundReadPermission(activity: ComponentActivity) {
        try {
            ...
            val intent = Intent("android.health.connect.action.REQUEST_PERMISSION").apply {
                putExtra(
                    "android.health.connect.extra.REQUEST_PERMISSIONS",
                    arrayOf(backgroundReadPermission)
                )
            }
            activity.startActivity(intent)
        } catch (e: Exception) {
